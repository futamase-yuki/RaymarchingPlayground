Shader "Unlit/My_1_RaymarchingSponge"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float sdCross( in float3 p )
            {
              float da = maxcomp(float3(abs(p.xy), 0));
              float db = maxcomp(float3(abs(p.yz), 0));
              float dc = maxcomp(float3(abs(p.zx), 0));
              return min(da,min(db,dc))-1.0;
            }
            float scene( float3 p )
            {
                float d = sdBox(p, float3(2,2,2));

                float s = 1.0;
                for (int m=0; m<1; m++)
                {
                    float3 a = mod(p*s, float3(4., 4., 4.)) - 2;
                    s*=3.0;
      ...
      // d = max(d,c);
            ...
            float4 frag(v2f input): SV_Target
            {
                ...
                for (int i = 0; i < 1; i++)
                {
                    ...
                    col += raymarching(cameraOrigin, ray, hit, reflectionAttenuation);
