Shader "Unlit/12_RaymarchingBoolean"
{
    Properties
    {
        _ObjectAlbedo ("Object Albedo", Color) = (1, 0, 0, 1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float dUnionObject(float3 p)
            {
                float box = sdBox(p - float3(0, 0.5, 0), float3(1.5, 0.5, 1.5));
                float sphere = sdSphere(p - float3(0, 1, 0), 1);
                return opUnion(box, sphere);
            }

            float dSubtractionObjectA(float3 p)
            {
                float box = sdBox(p - float3(5, 0.5, 0), float3(1.5, 0.5, 1.5));
                float sphere = sdSphere(p - float3(5, 1, 0), 1);
                return opSubtraction(box, sphere);
            }

            float dSubtractionObjectB(float3 p)
            {
                float box = sdBox(p - float3(10, 0.5, 0), float3(1.5, 0.5, 1.5));
                float sphere = sdSphere(p - float3(10, 1, 0), 1);
                return opSubtraction(sphere, box);
            }

            float dIntersectionObject(float3 p)
            {
                float box = sdBox(p - float3(15, 0.5, 0), float3(1.5, 0.5, 1.5));
                float sphere = sdSphere(p - float3(15, 1, 0), 1);
            ...
            {
                float d = dUnionObject(p);
                d = min(d, dSubtractionObjectA(p));
                d = min(d, dSubtractionObjectB(p));
                d = min(d, dIntersectionObject(p));
                d = min(d, dFloor(p));
                return d;
            }
