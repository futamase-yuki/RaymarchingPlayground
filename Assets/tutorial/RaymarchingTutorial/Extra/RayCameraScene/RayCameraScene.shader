Shader "Unlit/RayCameraScene1Pass"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float isHitHitPoints[20];
            float3 hitPoitns[20];
            float3 rayFrom[20];
            float3 rayTarget[20];

            float dRay(float3 p, int i, bool hitPoint)
            {
                float d = sdLine(p, rayFrom[i], rayFrom[i] + normalize(rayTarget[i] - rayFrom[i]) * 10.0, 0.0035 * length(p - _WorldSpaceCameraPos));

                if (hitPoint && isHitHitPoints[i])
                {
                    d = min(d, sdSphere(p - hitPoitns[i], 0.1));
                }
            ...
            void frag(v2f input, out float4 color: SV_Target, out float depth: SV_Depth)
            {
                ...
                {
                    float2 gridCount = floor(float2(_QuadPixelCount * _QuadSize.x / _QuadSize.y, _QuadPixelCount));
                    float3 scale = float3(-1, 1, 0);
                    float3 leftTop = _QuadPos + _QuadSize * (scale + float3(1 / gridCount.x, -1 / gridCount.y, 0));
                    int progress = int(_RayScanProgress * gridCount.x * gridCount.y) - i;
                    float3 target = leftTop - _QuadSize * scale * 2 * float3(float(progress % gridCount.x), float(progress / int(gridCount.x)), 0) / float3(gridCount, 1);
                    ...
                    float3 q = from;// レイの先端の座標
                    float3 ray = normalize(target - from);
                    
                    for (int j = 0; j < 30; j++)
                    {
                        float d = dBall(q);// 最短距離を計算します
                        ...
                        t += d;// 最短距離だけレイを進めます
                        q = from + ray * t;// レイの先端の座標を更新します
                    }

                    isHitHitPoints[i] = hit;
                    hitPoitns[i] = q;
                    rayFrom[i] = from;
                    rayTarget[i] = target;
                }
                ...
                if (hit)
                {
                    ...
                    for (int i = 0; i < _RayCount; i++)
                    {
                        if (dRay(gizmoHit, i, false) < 0.0001)
                        {
                            id = i;
                        }
                    }
                    ...
                    if (id >= 0)
                    {
                        float alpha = exp(-2 / _RayCount * id);

                        if (gizmoDepth < min(sceneDepth, quadDepth))
                        {
                            col = lerp(col, _RayColorA.rgb, _RayColorA.a * alpha);
                        }
                        else
                        {
                            col = lerp(col, _RayColorB.rgb, _RayColorB.a * alpha);
                        }
                    }
                    else
                    {
                        col = lerp(col, _RayColorA.rgb, _RayColorA.a);
                    }
                }
