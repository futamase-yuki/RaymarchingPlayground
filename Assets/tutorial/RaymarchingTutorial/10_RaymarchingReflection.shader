Shader "Unlit/10_RaymarchingReflection"
{
    Properties
    {
        ...
        Pass
        {
                    ...
                    inout float3 reflectionAttenuation)
            {
                ...
                if (hit)
                {
                    ...
                    // 直接光の拡散反射
                    col += albedo * diffuse * shadow * (1 - metalness) * _LightColor0.rgb;
                    
                    // 直接光の鏡面反射
                    col += albedo * specular * shadow * metalness * _LightColor0.rgb;
                    ...
                    reflectionAttenuation *= albedo *
                            fresnelSphericalGaussian(f0, dot(ref, normal)) * invFog;
            ...
            float4 frag(v2f input): SV_Target
            {
                ...
                // ワールド空間のレイ
                float3 ray = mul(transpose((float3x3)UNITY_MATRIX_V), viewRay);
                ...
                for (int i = 0; i < 3; i++)
                {
                    col += reflectionAttenuation *
                            raymarching(cameraOrigin, ray, hit, reflectionAttenuation);
