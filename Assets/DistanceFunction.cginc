...
float sdTorus(in float3 p, in float3 t) {
...
float linedist(float3 p, float3 a, float3 b) {
    ...
    return distance(p, lerp(a,b,clamp(k,0.,1.)));
}
float apple(float3 p) {
    p.x += sin(dot(p, float3(4,2,5)))*0.02;
    p.z += sin(dot(p, float3(2,3,7)))*0.02;
    p.y += sin(dot(p, float3(2,6,2)))*0.01;
  
    float2 cords = float2(sqrt(pow(length(p.xy),2.)+0.005), p.z);
    float sphere1 = length(cords-float2(0.1,-0.1))-0.85;
    float sphere2 = length(cords-float2(0.6,0.6))-0.5;
    float sphere3 = length(cords-float2(0.4,-0.8))-0.3;
    float profile = smin(smin(sphere1, sphere2, 0.9), sphere3, 0.5);
    float linez = -1.2;
    float linesub1 = linedist(p, float3(0,-5,linez), float3(0,5,linez)) - 0.2;
    float linesub2 = linedist(p, float3(-5,0,linez), float3(5,0,linez)) - 0.2;
    //return linedist(float2(profile, p.x), float2(0), float2(-10,0))-0.1;
